	...
	<body>
		...
		<div id="show-stallWorker" class="content scaffold-show" role="main">
			...
			<ol class="property-list stallWorker">
				...
				<g:if test="${stallWorkerInstance?.user}">
				<li class="fieldcontain">
					<span id="user-label" class="property-label"><g:message code="stallWorker.user.label" default="User" /></span>
					
						<span class="property-value" aria-labelledby="user-label"><g:link controller="user" action="show" id="${stallWorkerInstance?.user?.id}">${stallWorkerInstance?.user?.encodeAsHTML()}</g:link></span>
