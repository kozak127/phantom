	...
	<body>
		...
		<div id="show-inEventObjectCreator" class="content scaffold-show" role="main">
			...
			<ol class="property-list inEventObjectCreator">
				...
				<g:if test="${inEventObjectCreatorInstance?.inEventObject}">
				<li class="fieldcontain">
					<span id="inEventObject-label" class="property-label"><g:message code="inEventObjectCreator.inEventObject.label" default="In Event Object" /></span>
					
						<span class="property-value" aria-labelledby="inEventObject-label"><g:link controller="inEventObject" action="show" id="${inEventObjectCreatorInstance?.inEventObject?.id}">${inEventObjectCreatorInstance?.inEventObject?.encodeAsHTML()}</g:link></span>
