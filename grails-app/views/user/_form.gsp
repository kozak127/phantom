...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'eventOrganizer', 'error')} ">
    <label for="eventOrganizer">
        <g:message code="user.eventOrganizer.label" default="EventOrganizer" />
        
    </label>
    
<ul class="one-to-many">
<g:each in="${userInstance?.eventOrganizer?}" var="o">
    <li><g:link controller="eventOrganizer" action="show" id="${o.id}">${o?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="eventOrganizer" action="create" params="['user.id': userInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'eventOrganizer.label', default: 'EventOrganizer')])}</g:link>
</li>
</ul>
